...
public class ParserTest {
   ...
   public void testParser() throws IOException, ParseException {
      try (Reader r = new InputStreamReader(ParserTest.class.getClassLoader().getResourceAsStream("sample_bank_account/bank.proto"))) {
         ...
         // User
         Descriptor message = assertMessage(input, 0, "User", 13, 1, 1);
         assertField(message, 0, Label.REQUIRED, Type.INT32, "id", 1);
         assertField(message, 1, Label.REPEATED, Type.INT32, "accountIds", 2);
         assertField(message, 2, Label.REQUIRED, Type.STRING, "name", 3);
         ...
         assertField(message, 0, Label.REQUIRED, Type.STRING, "street", 1);
         assertField(message, 1, Label.REQUIRED, Type.STRING, "postCode", 2);
         assertField(message, 2, Label.REQUIRED, Type.INT32, "number", 3);
         assertField(message, 3, Label.REQUIRED, Type.BOOL, "isCommercial", 4);
         ...
         assertField(message, 2, Label.REQUIRED, Type.FIXED64, "creationDate", 3);
         assertField(message, 3, Label.OPTIONAL, "Limits", "limits", 4);
         assertField(message, 4, Label.REQUIRED, "Limits", "hardLimits", 5);
         ...
         assertField(message, 4, Label.REQUIRED, Type.INT32, "accountId", 5);
         assertField(message, 5, Label.REQUIRED, Type.FIXED64, "date", 6);
         assertField(message, 6, Label.REQUIRED, Type.DOUBLE, "amount", 7);
         assertField(message, 7, Label.REQUIRED, Type.BOOL, "isDebit", 8);
         assertField(message, 8, Label.REQUIRED, Type.BOOL, "isValid", 9);
   ...
   private void assertReserved(Descriptor message, int... numbers) {
      assertArrayEquals(numbers, message.getReservedNumbers());
   }
